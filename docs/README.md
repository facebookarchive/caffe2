...
You need to have access to the master branch to be able to generate the docs from source and you also need access to the gh-pages branch to publish the docs. Since you often want the files simultaneously and since the folder structures between master and gh-pages differ so greatly, it is advised to clone each branch to different folders. Otherwise if you switch back and forth between gh-pages and master within the same local repo, you'll end up seeing folders in branches that shouldn't be there, and it can get really confusing once you start running Doxygen and generating thousands of new files.

```
git clone https://github.com/caffe2/caffe2 c2master && cd c2master && git checkout master
git clone https://github.com/caffe2/caffe2 c2docs && cd c2docs && git checkout gh_pages
```
...
The flow to manually create the API documents is simple with this script:

1. Goto your master branch folder
2. `cd docs && python process.py`

If you want to push these to caffe2.ai, go to your docs project:
1. Copy the files generated in build/docs to your gh-pages branch, commit, and push.
2. `doxygen-c` and `doxygen-python` both go in the root folder of `gh-pages`
3. `operators-catalogue.md` goes in `_docs` 
...
This catalogue is found on Caffe2.ai, but in order to update this file, the script `github.py` in the master branch at /caffe2/python/docs must be run and the output copied to `gh-pages:caffe2/_docs/operators-catalogue.md`.

```
cd ~/c2master/caffe2/python/docs/
python github.py > ~/c2docs/_docs/operators-catalogue.md
```
