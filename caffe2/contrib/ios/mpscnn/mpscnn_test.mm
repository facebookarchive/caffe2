...
void testMPSCNN() {
  ...
  {
    for (const auto C : std::vector<size_t>{1, 2, 3, 4, 8, 11}) {
...

  {
    for (const auto& shared : std::vector<bool>{{true, false}}) {
      for (const auto& array : std::vector<bool>{{true, false}}) {
        ...
        LOG(INFO) << "MPSCNNPRelu Test: " << shared;
        Workspace ws;
        const auto channels = array ? 12 : 3;
        {
          auto* t = ws.CreateBlob("X_cpu")->GetMutable<TensorCPU>();
          t->Resize(1, channels, 8, 13);
          CPUContext ctx;
          math::RandGaussian<float, CPUContext>(t->size(), 0, 1, t->mutable_data<float>(), &ctx);
        }
  ...
  {
    for (const auto& kernel : std::vector<size_t>{2, 4}) {
      for (const auto& stride : std::vector<size_t>{2, 4}) {
        for (const auto& pad : std::vector<size_t>{0, 1}) {

          for (auto adj = 0; adj < stride; ++adj) {
...

  {
    for (const auto array : std::vector<bool>{true, false}) {
      for (auto numInputs = 2; numInputs <= 4; numInputs++) {
        ...
        LOG(INFO) << "MPSCNNConcat Test";
        Workspace ws;
        for (auto i = 0; i < numInputs; ++i) {
          auto* t = ws.CreateBlob(cpu(i))->GetMutable<TensorCPU>();
          t->Resize(1, array ? (i + 1) * 4 : 4, 320, 180);
          CPUContext ctx;
          math::RandGaussian<float, CPUContext>(t->size(), 0, 1, t->mutable_data<float>(), &ctx);
        }
  ...
  {
    LOG(INFO) << "MPSConvTransposeRelu Test";
    Workspace ws;
    {
      auto* t = ws.CreateBlob("X_cpu")->GetMutable<TensorCPU>();
      t->Resize(1, 4, 320, 180);
      CPUContext ctx;
      math::RandGaussian<float, CPUContext>(t->size(), 0, 1, t->mutable_data<float>(), &ctx);
    }
    ...
    {
      auto& op = *(netdef.add_op());
      op.set_type("MPSCNNConvTransposeRelu");
      op.add_input("X_mtl");
      op.add_input("W");
      op.add_input("b");
      {
  ...
  {
    for (const auto& computeOp : std::vector<std::string>{"FC", "ConvTranspose", "Conv"}) {
