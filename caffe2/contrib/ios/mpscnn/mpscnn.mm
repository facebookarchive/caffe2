...
LaunchParams spatialPointwiseKernelLaunchParams(id<MTLComputePipelineState> pipeline,
  ...
  const auto threadgroupsPerGrid = MTLSizeMake(divRoundUp(im.width, threadsPerThreadgroup.width),
                                               divRoundUp(im.height, threadsPerThreadgroup.height),
                                               divRoundUp(im.featureChannels, 4));
...
auto computeMPSAlignOffset(int kernel, int pad) {
...
MPSTemporaryImage* createImage(const OperatorBase* op,
  ...
  const auto readCount = op->GetSingleArgument<int>(kMPSCNNReadCountArg, 1);
...
class CopyToMPSCNNOp final : public Operator<CPUContext> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    commandBuffer_ = Inputs().size() > 1
                         ? (__bridge id<MTLCommandBuffer>)(Inputs()[1]
                                                               ->template Get<MPSImageWrapper>()
                                                               .commandBuffer_)
                         : [getMPSCNNContext().commandQueue commandBuffer];
    output_ = createImage(this, commandBuffer_, height, width, channels);
    ...
    id<MTLComputePipelineState> state = getMPSCNNContext().getSpecializedPipelineState(
        channels > 4 ? @"copy_nchw_to_metal" : @"copy_nchw_to_metal_nonarray",
        {{ushort(channels), ushort(height), ushort(width), 0}});
...
OPERATOR_SCHEMA(CopyToMPSCNN).NumInputs(1, 2).NumOutputs(1);
...
class CopyFromMPSCNNOp final : public Operator<CPUContext> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    const id<MTLCommandBuffer> commandBuffer =
        (__bridge id<MTLCommandBuffer>)(Inputs()[0]->Get<MPSImageWrapper>().commandBuffer_);

    VLOG(2) << "X: " << X.height << ", " << X.width << ", " << X.featureChannels;
    CAFFE_ENFORCE_EQ(X.numberOfImages, 1);
    const int Xbytes = X.height * X.width * X.featureChannels * sizeof(float);
    if (!outputBuffer_ || outputBuffer_.length != Xbytes) {
      outputBuffer_ = [getMPSCNNContext().device
          newBufferWithLength:X.height * X.width * X.featureChannels * sizeof(float)
                      options:MTLResourceOptionCPUCacheModeDefault];
    }
    caffe2::Timer t;
    id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
    id<MTLComputePipelineState> state = getMPSCNNContext().getSpecializedPipelineState(
        X.featureChannels > 4 ? @"copy_metal_to_nchw" : @"copy_metal_to_nchw_nonarray",
        {{ushort(X.featureChannels), ushort(X.height), ushort(X.width), 0}});
    ...
    Output(0)->Resize(1, X.featureChannels, X.height, X.width);
    Output(0)->mutable_data<float>();

    CAFFE_ENFORCE_EQ(outputBuffer_.length, Output(0)->nbytes());
    memcpy(Output(0)->mutable_data<float>(), [outputBuffer_ contents], outputBuffer_.length);
...
OPERATOR_SCHEMA(CopyFromMPSCNN).NumInputs(1).NumOutputs(1);
...
class MPSCNNPackedInt8BGRANHWCToNCHWCStylizerPreprocessOp final : public Operator<CPUContext> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    output_ = createImage(this, commandBuffer_, H, W, 3);

    id<MTLComputeCommandEncoder> encoder = [commandBuffer_ computeCommandEncoder];
    id<MTLComputePipelineState> state = getMPSCNNContext().getSpecializedPipelineState(
        @"preprocess_stylizer", {{noiseSize, 0, 0, 0}});
...
class MPSCNNNeuronOp final : public Operator<CPUContext> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    output_ = createImage(this, commandBuffer, X.height, X.width, X.featureChannels);
...
class MPSCNNConvOp final : public ConvPoolOpBase<CPUContext> {
 public:
  ...
  bool RunOnDeviceWithOrderNCHW() override {
    ...
    output_ = createImage(this, commandBuffer, output_height, output_width, output_channels);
...
class MPSCNNPadImageOp final : public ConvPoolOpBase<CPUContext> {
 public:
  ...
  bool RunOnDeviceWithOrderNCHW() override {
    ...
    output_ = createImage(this, commandBuffer, output_height, output_width, X.featureChannels);

    CAFFE_ENFORCE_EQ(output_.height, output_height);
    CAFFE_ENFORCE_EQ(output_.width, output_width);
    id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
    id<MTLComputePipelineState> state = getMPSCNNContext().getPipelineState(
        output_.featureChannels <= 4 ? @"reflection_padding_nonarray" : @"reflection_padding");
    [encoder setComputePipelineState:state];
    [encoder setTexture:[X texture] atIndex:0];
  ...
  bool RunOnDevice() override {
    ...
    id<MTLCommandBuffer> commandBuffer =
        (__bridge id<MTLCommandBuffer>)(Inputs()[0]
                                            ->template Get<MPSImageWrapper>()
                                            .commandBuffer_);
    output_ = createImage(this, commandBuffer, X0.height, X0.width, X0.featureChannels);
    ...
    id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
    id<MTLComputePipelineState> state = getMPSCNNContext().getPipelineState(
        X0.featureChannels <= 4 ? @"elementwise_add_nonarray" : @"elementwise_add");
...
class MPSCNNFullyConnectedOp final : public Operator<CPUContext> {
 public:
      ...
      MPSCNNConvolutionDescriptor* desc =
          [MPSCNNConvolutionDescriptor cnnConvolutionDescriptorWithKernelWidth:X.width
                                                         ...
                                                         outputFeatureChannels:output_channels
                                                                  neuronFilter:Neuron::t()];
      fc_ = [[MPSCNNFullyConnected alloc] initWithDevice:getMPSCNNContext().device
                                   convolutionDescriptor:desc
                                           kernelWeights:refilter.data()
                                               biasTerms:b.template data<float>()
                                                   flags:MPSCNNConvolutionFlagsNone];
    }
    output_ = createImage(this, commandBuffer, 1, 1, output_channels);
  ...
  MPSCNNFullyConnected* fc_{nullptr};
  MPSTemporaryImage* output_{nullptr};
};
...
class MPSCNNConvTransposeOp final : public ConvTransposeUnpoolBase<CPUContext> {
 public:
  ...
  bool RunOnDeviceWithOrderNCHW() override {
    ...
    if (!conv_) {
      caffe2::Timer consT;
      // refilter[output_channels][kH][kW][input_channels]
      std::vector<float> refilter(output_channels * input_channels * kH * kW);
      auto* filter_ = filter.template data<float>();
      // filter[input_channels][output_channels][kH][kW]
      for (auto output_channel = 0; output_channel < output_channels; ++output_channel) {
        for (auto input_channel = 0; input_channel < input_channels; ++input_channel) {
          for (auto kh = 0; kh < kH; ++kh) {
            for (auto kw = 0; kw < kW; ++kw) {
              // W_rot_repro[oc, ic, kh, kw] = W[ic, oc, kH - 1 - kh, kW - 1 - kw]
              refilter[output_channel * kH * kW * input_channels + kh * kW * input_channels +
                       kw * input_channels + input_channel] =
                  filter_[input_channel * output_channels * kH * kW + output_channel * kH * kW +
                          (kH - 1 - kh) * kW + (kW - 1 - kw)];
            }
          }
        }
      ...
      [conv_ setEdgeMode:MPSImageEdgeModeZero];
      MPSOffset offset;
      // Note that since we handle padding in the upscaling stage,
      // we pass a zero-padding input here.
      offset.x = computeMPSAlignOffset(kW, 0);
      offset.y = computeMPSAlignOffset(kH, 0);
      offset.z = 0;
      [conv_ setOffset:offset];
    ...
    CAFFE_ENFORCE_EQ(conv_.outputFeatureChannels, output_channels);
    CAFFE_ENFORCE_EQ(conv_.kernelWidth, kW);
    CAFFE_ENFORCE_EQ(conv_.kernelHeight, kH);

    int upscaled_height = this->stride_h_ * (X.height - 1) + kH + (kH - 1) - this->pad_b_ -
                          this->pad_t_ + this->adj_h_;
    int upscaled_width = this->stride_w_ * (X.width - 1) + kW + (kW - 1) - this->pad_l_ -
                         this->pad_r_ + this->adj_w_;
    ...
    VLOG(2) << "Output width:" << output_width;
    VLOG(2) << "Output channels:" << output_channels;
    VLOG(2) << "Upscaled height: " << upscaled_height;
    VLOG(2) << "Upscaled width:" << upscaled_width;
    VLOG(2) << "Upscaled channels:" << X.featureChannels;

    {
      caffe2::Timer ut;
      upscaled_ = createImage(this, commandBuffer, upscaled_height, upscaled_width, input_channels);

      id<MTLComputePipelineState> state = getMPSCNNContext().getSpecializedPipelineState(
          X.featureChannels > 4 ? @"convtranspose_upscale" : @"convtranspose_upscale_4",
          {{ushort(this->kernel_w_), ushort(this->stride_w_), ushort(this->pad_b_), 0}});
      id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
      [encoder setComputePipelineState:state];
      [encoder setTexture:[X texture] atIndex:0];
      [encoder setTexture:[upscaled_ texture] atIndex:1];
      const auto& launchParams = spatialPointwiseKernelLaunchParams(state, upscaled_);
      [encoder dispatchThreadgroups:launchParams.threadgroupsPerGrid
              threadsPerThreadgroup:launchParams.threadsPerThreadgroup];
      [encoder endEncoding];
      X.readCount -= 1;
      VLOG(2) << "MPSCNNConvTranspose upscaling took: " << ut.MilliSeconds();
    }

    output_ = createImage(this, commandBuffer, output_height, output_width, output_channels);
    [conv_ encodeToCommandBuffer:commandBuffer sourceImage:upscaled_ destinationImage:output_];
    Outputs()[0]->template GetMutable<MPSImageWrapper>()->x = (__bridge void*)output_;
    Outputs()[0]->template GetMutable<MPSImageWrapper>()->commandBuffer_ =
        (__bridge void*)commandBuffer;
  ...
  MPSCNNConvolution* conv_{nullptr};
  MPSTemporaryImage* output_{nullptr};
  MPSTemporaryImage* upscaled_{nullptr};
};

// No-op init
#define INIT_CONV_TRANSPOSE_NEURON_OP(name, neuron)           \
  REGISTER_CPU_OPERATOR(name, MPSCNNConvTransposeOp<neuron>); \
  OPERATOR_SCHEMA(name).NumInputs(3).NumOutputs(1);

INIT_CONV_TRANSPOSE_NEURON_OP(MPSCNNConvTranspose, EmptyNeuronInit);
INIT_CONV_TRANSPOSE_NEURON_OP(MPSCNNConvTransposeRelu, ReluNeuronInit);
 ...
 public:
  ...
  bool RunOnDevice() override {
    ...
    if (X.featureChannels <= 4) {
      kernel = @"instance_norm_nonarray";
    } else {
      CAFFE_ENFORCE_GT(X.featureChannels, 4);
      kernel = @"instance_norm";
    }

    caffe2::Timer t;
    id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
    id<MTLComputePipelineState> state = getMPSCNNContext().getPipelineState(kernel);
...
class MPSCNNPReluOp final : public Operator<CPUContext> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    NSString* kernel = nil;
    if (X.featureChannels <= 4) {
      kernel = @"prelu_nonshared_nonarray";
    } else {
      CAFFE_ENFORCE_GT(X.featureChannels, 4);
      kernel = @"prelu_nonshared";
    }

    caffe2::Timer t;
    id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
    id<MTLComputePipelineState> state = getMPSCNNContext().getSpecializedPipelineState(
        kernel, {{ushort(scale.size() == X.featureChannels), 0, 0, 0}});
...
class MPSCNNSpatialBNOp final : public SpatialBNOp<CPUContext> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
    id<MTLComputePipelineState> state = getMPSCNNContext().getSpecializedPipelineState(
        X.featureChannels > 4 ? @"affine" : @"affine_nonarray",
        {{ushort(X.featureChannels), 0, 0, 0}});
...
class MPSCNNConcatOp final : public Operator<CPUContext> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    std::array<ushort, 4> channels = {{0, 0, 0, 0}};
    ...
    output_ = createImage(this, commandBuffer, X(0).height, X(0).width, channelCount);
    caffe2::Timer t;
    id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
...
bool tryFuseAdjacentOps(const OperatorDef& currentOp,
  ...
  static const std::map<std::pair<std::string, std::string>, std::string> fusionOpportunities = {{
      ...
      {{"MPSCNNConvTranspose", "MPSCNNRelu"}, "MPSCNNConvTransposeRelu"},
