...
std::function<void(void *)> deleterFor(at::Tensor t) {
  ...
  return [t](void *ptr) mutable { t.reset(); };
}

template <class Context> class ATenOp : public Operator<Context> {
public:
  ...
  bool RunOnDevice() override { return run_op(); }
...
#undef DEFINE_IF
  ...
  // to lose precision
  double extract_d(const at::Scalar &s) { return s.toDouble(); }
  int64_t extract_i(const at::Scalar &s) { return s.toLong(); }

  void assignTo(Tensor<Context> *dst, at::Type &inferred_type,
                at::Scalar scalar) {
...
#undef DEFINE_CASE
  ...
  template <typename T> void assignToValue(Tensor<Context> *dst, T v) {
  ...
  template <typename T> T readAttribute(const std::string &name) {
...
#define DEFINE_IF(type, aten)                                                  \
  ...
  at::Type *readTypeAttribute(const std::string &name) {
    ...
    return &stringToType(
        OperatorBase::GetSingleArgument<std::string>(name, ""));
  }
};
}
