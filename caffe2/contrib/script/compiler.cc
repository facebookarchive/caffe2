...
struct FunctionDefinition {
  FunctionDefinition(Def tree)
  : tree(new Def(tree))
  , net_def(new NetDef()) {}

  FunctionDefinition(std::unique_ptr<NetDef> def)
  : tree(nullptr)
  , net_def(std::move(def)) {
...
struct DefCompiler {
  ...
  void renameOp(
    ...
    for(size_t i = 0; i < new_op->input().size(); i++) {
      ...
      if(!isExtern && !defined) {
        throw ErrorReport(apply) << " unexpected undefined name '" << name
    ...
    if(new_op->type() == "If" || new_op->type() == "While") {
  ...
  void emitFunctionCall(Apply & apply,
    ...
    if(inputs.size() != fn.inputs.size()) {
      throw ErrorReport(apply) << fname << " expected " << fn.inputs.size()
        << " values but recieved " << inputs.size();
    }
  ...
  void expectOutputs(const TreeRef & tree,
                     const std::vector<std::string> & outputs,
                     size_t size) {
    if(outputs.size() != size) {
      throw ErrorReport(tree) << "expected operator to produce " << outputs.size()
        << " outputs but it produced " << size;
    }
  }
  void appendOutputs(const TreeRef & tree,
                     OperatorDef * op,
                     const std::vector<std::string> & outputs,
                     size_t size) {
 ...
 private:
  ...
  std::unordered_map<
      std::string,
      std::function<void(DefCompiler *, const Apply&, const std::vector<std::string> & outputs)>>
...
struct CompilationUnitImpl {
  ...
  void defineFunction(const Def& def) {
    ...
    DefCompiler c(
        functions.emplace(def.name().name(), FunctionDefinition(def)).first->second,
        functions);
    c.run();
  }
  ...
  void defineExtern(const std::string & name,  std::unique_ptr<NetDef> net_def) {
    ...
    functions.emplace(name, std::move(net_def));
  }
...
void CompilationUnit::defineExtern(const std::string & name, std::unique_ptr<NetDef> nd) {
