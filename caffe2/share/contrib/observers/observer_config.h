...
/*
  netSampleRate_ == 1 && operatorNetSampleRatio_ == 1 : Log operator metrics in every iteration
  netSampleRate_ == 1 && operatorNetSampleRatio_ == 0 : Log net metrics in every iterationn
  netSampleRate_ == n && operatorNetSampleRatio_ == 1 : Log operator metrics every n iterations
  netSampleRate_ == n && operatorNetSampleRatio_ == 0 : Log net metrics every n iterations
  netSampleRate_ == n && operatorNetSampleRatio_ == m : Log net metrics every n iterations (except n * m iterations)
...
public:
  ...
  static int getNetSampleRate() { return netSampleRate_;}
  static int getOpoeratorNetSampleRatio() { return operatorNetSampleRatio_;}
  static int getSkipIters() { return skipIters_;}
