...
#ifndef CAFFE2_OPERATORS_REDUCE_MEAN_OPS_H_
#define CAFFE2_OPERATORS_REDUCE_MEAN_OPS_H_
...
class ReduceMeanOp final : public Operator<Context> {
 public:
    ...
    for (int id = axes_.size() - 1; id >= 0; id--) {
      ...
      for (int i = 0; i < front; i++) {
        for (int k = 0; k < back; k++) {
          ...
          temp_buf[i * back + k] = sum / reduced_dim;
        }
      }
  ...
  bool RunOnDevice() override {
    if (Input(0).template IsType<float>()) {
      ...
      CAFFE_THROW(
          "ReduceMean operator only supports 32-bit float, but",
          " input was of type ",
          Input(0).meta().name());
    }
  }
...
#endif // CAFFE2_OPERATORS_REDUCE_MEAN_OPS_H_
