...
CAFFE2_DEFINE_bool(warp, false,
    "If warp is set, warp the images to square.");
CAFFE2_DEFINE_int(num_threads, -1,
...
void ConvertImageDataset(
  ...
  for (auto i = 0; i < lines.size(); i++) {
    ...
    auto key_len = snprintf(key_cstr,
                        sizeof(key_cstr),
                        "%08d_%s",
                        i,
                        lines[i].first.c_str());
