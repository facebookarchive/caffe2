...
DOWNLOAD_BASE_URL="https://github.com/bwasti/models/raw/master/"
DOWNLOAD_COLUMNS = 70
...
def downloadFromURLToFile(url, filename):
    try:
        ...
        raise Exception("Could not download model. [HTTP Error] {code}: {reason}.".format(code=e.code, reason=e.reason))
    except URLError as e:
        raise Exception("Could not download model. [URL Error] {reason}.".format(reason=e.reason))
...
def downloadModel(model, args):
    ...
    if args.install:
        model_folder = '{dir_path}/{folder}'.format(dir_path=dir_path, folder=model)

    # Check if that folder is already there
    if os.path.exists(model_folder) and not os.path.isdir(model_folder):
        if not args.force:
            raise Exception("Cannot create folder for storing the model, there exists a file of the same name.")
        else:
            print("Overwriting existing file! ({filename})".format(filename=model_folder))
    ...
    for f in ['predict_net.pb', 'init_net.pb']:
        try:
            downloadFromURLToFile(getURLFromName(model, f), '{folder}/{f}'.format(folder=model_folder, f=f))
        except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--install', help='Install the model.', action='store_true')
    parser.add_argument('-f', '--force', help='Force a download/installation.', action='store_true')
