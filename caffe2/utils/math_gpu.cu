...
void Gemm<float16, CUDAContext>(
    const CBLAS_TRANSPOSE TransA, const CBLAS_TRANSPOSE TransB,
    const int M, const int N, const int K, const float alpha, const float16* A,
    const float16* B, const float beta, float16* C, CUDAContext* context,
    TensorProto::DataType math_type) {
  // Note that cublas follows fortran order, so the order is different from
  // the cblas convention.
  int lda = (TransA == CblasNoTrans) ? K : M;
  ...
  if (math_type == TensorProto_DataType_FLOAT) {
    CUBLAS_CHECK(cublasSgemmEx(context->cublas_handle(), cuTransB, cuTransA,
        N, M, K,
        &alpha, B, CUDA_R_16F, ldb,
        A, CUDA_R_16F, lda,
        &beta, C, CUDA_R_16F, N));
    ...
    // call cublasHgemm
    CUBLAS_CHECK(cublasHgemm(context->cublas_handle(), cuTransB, cuTransA,
        N, M, K, &alpha_fp16, (const __half*)B, ldb, (const __half*)A, lda, &beta_fp16, (__half*)C, N));
  } else {
    // fail
...
void Gemv<float16, CUDAContext>(
    const CBLAS_TRANSPOSE TransA, const int M, const int N, const float alpha,
    const float16* A, const float16* x, const float beta, float16* y,
    CUDAContext* context,
    TensorProto::DataType math_type) {
  cublasOperation_t cuTransA =
      (TransA == CblasNoTrans) ? CUBLAS_OP_T : CUBLAS_OP_N;
  ...
  if (math_type == TensorProto_DataType_FLOAT) {
    CUBLAS_CHECK(cublasSgemmEx(context->cublas_handle(), cuTransA, CUBLAS_OP_N,
                               m, 1, k,
                               &alpha, A, CUDA_R_16F, LDA,
                               x, CUDA_R_16F, k,
                               &beta, y, CUDA_R_16F, LDC));
  } else if (math_type == TensorProto_DataType_FLOAT16) {
    __half alpha_fp16;
    alpha_fp16.x = convert::To<float,float16>(alpha).x;
    __half beta_fp16;
    ...
    CUBLAS_CHECK(cublasHgemm(context->cublas_handle(), cuTransA, CUBLAS_OP_N,
                             m, 1, k,
                             &alpha_fp16, (const __half*)A, LDA,
                             (const __half*)x, k,
...
namespace {
template <typename T>
__global__ void UniformShift(const int N, const float min, const float max,
                             T* x) {
  float scale = max - min;
  CUDA_1D_KERNEL_LOOP(i, N) {
...
void Dot<float16, CUDAContext>(
    const int n, const float16* a, const float16* b, float16* y,
    CUDAContext* context) {
  float16 result;
  // execute with 32-bit math
  CUBLAS_CHECK(cublasDotEx(context->cublas_handle(), n, a, CUDA_R_16F, 1, b, CUDA_R_16F, 1, &result, CUDA_R_16F, CUDA_R_32F));
...
void Select<float16, CUDAContext>(
      const int N, const int D, const float16* x, const int* idx, float16* y,
      CUDAContext* context) {
  SelectKernel<float16><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS,
                        0, context->cuda_stream()>>>(N, D, x, idx, y);
}
...
__global__ void ScaleKernel(
    const int n, const float alpha, const T* x, T* y) {
  CUDA_1D_KERNEL_LOOP(i, n) {
...
template <>
__global__ void ScaleKernelDeviceAlpha(
    const int n, const float* alpha, const float16* x, float16* y) {
  CUDA_1D_KERNEL_LOOP(i, n) {
    y[i] = convert::To<float,float16>(convert::To<float16,float>(x[i]) * (*alpha));
  }
}
...
void Scale<float16, CUDAContext>(
    const int n, const float alpha, const float16 *x, float16* y,
    CUDAContext* context) {
  ScaleKernel<float16><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS,
                       0, context->cuda_stream()>>>(n, alpha, x, y);
}
...
void Scale<float16, CUDAContext>(
    const int n, const float* alpha, const float16 *x, float16* y,
    CUDAContext* context) {
  ScaleKernelDeviceAlpha<float16><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS,
                       0, context->cuda_stream()>>>(n, alpha, x, y);
}
...
void Axpy<double, CUDAContext>(
  ...
  CUBLAS_ENFORCE(cublasDaxpy(context->cublas_handle(), N, &alpha_d, X, 1, Y, 1));
}

template <>
void Axpy<float16, CUDAContext>(const int N, const float alpha, const float16* X,
                                float16* Y, CUDAContext* context) {

  CUBLAS_CHECK(cublasAxpyEx(context->cublas_handle(), N, &alpha, CUDA_R_16F,
                            X, CUDA_R_16F, 1,
                            Y, CUDA_R_16F, 1,
                            CUDA_R_32F));
}
...
void Axpy<float16, CUDAContext>(
    const int n, const float* alpha, const float16* X,
    float16* Y, CUDAContext* context) {
  AxpyKernel<float16><<<CAFFE_GET_BLOCKS(n), CAFFE_CUDA_NUM_THREADS,
                       0, context->cuda_stream()>>>(n, alpha, X, Y);
}
